...
const Navbar: React.FC<NavbarProps> = ({ theme, toggleTheme, className = '' }) => {
  const { wings, isSectionVisible } = useContent();
  ...
  if (!isSectionVisible('layout', 'navbar')) return null;
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center relative">
        ...
        <div className="hidden lg:flex items-center absolute left-1/2 -translate-x-1/2 bg-white/80 dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-full px-8 py-3 backdrop-blur-lg shadow-lg shadow-slate-200/50 dark:shadow-black/50 z-20">
          <div className="flex items-center space-x-8">
            {isSectionVisible('layout', 'navHome') && (
            ...
            {isSectionVisible('layout', 'navWings') && (
              <div ref={wingsDropdownRef} className="relative">
                ...
                >
                  <span>WINGS</span>
                  <ChevronDown size={10} className={`transition-transform duration-300 ${wingsDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
            ...
            {isSectionVisible('layout', 'navPortfolio') && (
            ...
            {isSectionVisible('layout', 'navCompany') && (
        ...
        <div className="hidden lg:flex items-center space-x-6 z-20">
          ...
          >
            {theme === 'dark' ? (
              <><Sun size={12} className="text-yellow-400" /><span>LIGHT</span></>
            ) : (
              <><Moon size={12} className="text-brand-500" /><span>DARK</span></>
            )}
          </button>

          <div className="h-4 w-[1px] bg-slate-300 dark:bg-white/10"></div>

          {isSectionVisible('layout', 'navDashboard') && (
            <Link
              to="/dashboard"
          ...
          {isSectionVisible('layout', 'navContact') && (
