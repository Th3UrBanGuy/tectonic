...
import { Send, Search, ArrowLeft, Paperclip, MoreVertical, Phone, Video, Check, CheckCheck, Plus, Trash2, X } from 'lucide-react';
...
const ChatSystem = () => {
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        setMessages(prev => ({
            ...prev,
            [selectedUser.id]: [...(prev[selectedUser.id] || []), optimistic],
        }));
        ...
        try {
            await fetch('/api/chat/messages', {
                ...
                body: JSON.stringify({ threadId: selectedUser.id, text }),
            });
        } catch {
            // silent — message still shows optimistically
        }
        ...
        setTimeout(() => {
            ...
            const reply: ChatMsg = {
                ...
                senderAvatar: selectedUser.avatar || selectedUser.name.charAt(0),
    ...
    return (
        <div className="w-full h-[600px] md:h-[calc(100vh-250px)] flex bg-slate-50 dark:bg-black rounded-2xl overflow-hidden border border-slate-200/50 dark:border-white/[0.05] shadow-2xl">
            ...
            <div className={`w-full md:w-80 flex flex-col bg-white dark:bg-slate-950/50 backdrop-blur-xl border-r border-slate-200/50 dark:border-white/[0.05] ${selectedUser ? 'hidden md:flex' : 'flex'}`}>
                {/* Header */}
                <div className="p-5 border-b border-slate-200/50 dark:border-white/[0.05] space-y-3">
                    <div className="flex items-center justify-between">
                        <h2 className="font-bold text-slate-900 dark:text-white text-sm">Conversations</h2>
                        <button
                            onClick={() => setShowNewChat(true)}
                            className="p-1.5 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg text-cyan-600 dark:text-cyan-400 transition-colors"
                ...
                <div className="flex-1 overflow-y-auto">
                    {loadingThreads ? (
                        ...
                        <div className="text-center py-10 text-slate-400 dark:text-slate-600">
                            ...
                            <button onClick={() => setShowNewChat(true)} className="text-xs text-cyan-500 mt-2 hover:underline">Start one</button>
                        </div>
                    ) : (
                        filteredThreads.map((user) => (
                            <motion.div
                                ...
                                className={`group flex items-center gap-3 p-4 cursor-pointer transition-all border-b border-slate-100/50 dark:border-white/[0.03] ${selectedUser?.id === user.id
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-baseline justify-between mb-1">
                                        ...
                                        {user.time && <span className="text-xs text-slate-500 dark:text-slate-500 ml-2 flex-shrink-0">{user.time}</span>}
                                    </div>
                                    <div className="flex items-center justify-between gap-2">
                                        <p className="text-xs text-slate-600 dark:text-slate-400 truncate">{user.lastMsg || 'No messages yet'}</p>
                                        {(user.unread ?? 0) > 0 && (
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); handleDeleteThread(user.id); }}
            ...
            <div className={`flex-1 flex flex-col bg-white dark:bg-slate-950 ${selectedUser ? 'flex' : 'hidden md:flex'}`}>
                {selectedUser ? (
                    <>
                        ...
                        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200/50 dark:border-white/[0.05] bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl">
                            ...
                            <div className="flex items-center gap-1">
                                <motion.button whileTap={{ scale: 0.9 }} className="p-2 text-slate-600 dark:text-slate-400 hover:text-brand-500 dark:hover:text-brand-400 transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-white/5">
                                    <Phone size={18} />
                                </motion.button>
                                <motion.button whileTap={{ scale: 0.9 }} className="p-2 text-slate-600 dark:text-slate-400 hover:text-brand-500 dark:hover:text-brand-400 transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-white/5">
                                    <Video size={18} />
                        ...
                        {/* Messages Area */}
                        <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 bg-slate-50/50 dark:bg-black">
                                ...
                                {loadingMessages ? (
                                    ...
                                    <AnimatePresence initial={false}>
                                        {(messages[selectedUser.id] || []).map((msg, index) => {
                                            const msgList = messages[selectedUser.id] || [];
                                            const showAvatar = index === 0 || msgList[index - 1]?.isMe !== msg.isMe;
                                            return (
                                                <motion.div
                                                    key={msg.id}
                                                    layout
                                                ...
                                                >
                                                    {!msg.isMe && (
                                                        <div className={`w-7 h-7 rounded-full bg-gradient-to-br from-brand-500 to-cyan-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 shadow-md ${showAvatar ? 'opacity-100' : 'opacity-0'}`}>
                                                            {msg.senderAvatar || selectedUser.avatar || selectedUser.name.charAt(0)}
                                                        </div>
                                                    )}
                                                    <div className={`group flex flex-col ${msg.isMe ? 'items-end' : 'items-start'} max-w-[70%] sm:max-w-[55%]`}>
                                                        ...
                                                        <div className="flex items-center gap-1.5 mt-1 px-1">
                                                            <span className="text-[10px] text-slate-500 dark:text-slate-600">{formatTime(msg.createdAt)}</span>
                                                            {msg.isMe && (
                                    ...
                                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex items-end gap-2">
                                        ...
                                        <div className="px-4 py-3 bg-white dark:bg-white/[0.05] rounded-2xl rounded-bl-sm border border-slate-200/50 dark:border-white/[0.08] flex gap-1">
                                            {[0, 0.2, 0.4].map((delay, i) => (
                                                <motion.div key={i} animate={{ y: [0, -4, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay }} className="w-2 h-2 bg-slate-400 dark:bg-slate-600 rounded-full" />
                                            ))}
                                        </div>
                                    </motion.div>
                                )}
                        ...
                        <form onSubmit={handleSend} className="p-4 border-t border-slate-200/50 dark:border-white/[0.05] bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl">
                            <div className="flex items-center gap-2">
                                <div className="flex-1 flex items-center gap-2 bg-slate-100 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08] rounded-xl px-4 py-2.5 focus-within:ring-2 focus-within:ring-brand-500/30 focus-within:border-brand-500/50 transition-all">
                                    ...
                                    />
                                    {!message.trim() && (
                                        <motion.button whileTap={{ scale: 0.9 }} type="button" className="text-slate-500 dark:text-slate-600 hover:text-brand-500 dark:hover:text-brand-400 transition-colors">
                                            <Paperclip size={18} />
                                        </motion.button>
                                    )}
                                </div>
                    ...
                    <div className="hidden md:flex flex-col items-center justify-center h-full text-slate-400 dark:text-slate-600">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-600 mt-1">Or start a new one with the + button</p>
                    </div>
                )}
            </div>
                ...
                {showNewChat && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl w-full max-w-sm p-6 shadow-2xl"
                        >
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-bold text-slate-900 dark:text-white">New Conversation</h3>
                                <button onClick={() => setShowNewChat(false)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white">
                                    <X size={20} />
                                </button>
                            </div>
                            <input
                                ...
                                className="w-full bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 text-slate-900 dark:text-white focus:border-cyan-500/50 focus:outline-none transition-colors mb-4"
                                autoFocus
                            />
                            <button
                                onClick={handleCreateThread}
                                disabled={!newChatName.trim()}
                                className="w-full py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg font-bold transition-colors disabled:opacity-50"
                            >
                                Create Conversation
                            </button>
                        </motion.div>
