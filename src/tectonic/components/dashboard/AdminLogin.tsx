...
import { ArrowRight, Lock, User, ArrowLeft, Loader2 } from 'lucide-react';
import { Link, useNavigate, useLocation, Navigate } from '@/tectonic/lib/router';
import { useAuth } from '../AuthContext';
import TectonicLogo from '../layout/TectonicLogo';
import ThemeToggle from '../ui/ThemeToggle';

/**
 * AdminLogin — the single login page for the Tectonic admin portal.
 *
 * Design: glassmorphism card with "Portal" title, TECTONIC logo badge,
 * teal accent line, "Secure authentication required" subtitle, and an
 * "Authenticate" button.
 *
 * Auth: real JWT authentication via useAuth().login() → POST /api/auth/login.
 * On success, navigates to the originally-requested page (or /dashboard).
                    ...
                    <div className="relative bg-white/70 dark:bg-white/[0.02] backdrop-blur-xl border border-slate-200/50 dark:border-white/[0.05] rounded-3xl p-8 md:p-12 shadow-2xl shadow-slate-200/50 dark:shadow-black/50">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <motion.button
                                ...
                                type="submit"
                                disabled={isSubmitting}
                                className="w-full mt-8 group relative overflow-hidden bg-slate-900 dark:bg-white/[0.05] hover:bg-slate-800 dark:hover:bg-white/[0.08] border border-transparent dark:border-white/[0.1] hover:dark:border-brand-500/50 rounded-xl py-4 transition-all duration-500 shadow-xl shadow-brand-500/10 disabled:opacity-60 disabled:cursor-not-allowed"
                            >
                                {/* Hover Gradient */}
                                ...
                                <div className="relative flex items-center justify-center gap-3">
                                    {isSubmitting ? (
                                        <>
                                            <Loader2 className="w-5 h-5 text-brand-400 dark:text-brand-500 animate-spin" />
