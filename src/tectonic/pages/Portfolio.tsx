...
const Portfolio = () => {
  ...
  const { projects: dbProjects, portfolioContent, contentLoaded, isSectionVisible } = useContent();
  const projects = dbProjects as ProjectItem[];
  const PORTFOLIO_CONTENT = portfolioContent || { header: { badge: 'OUR WORK', title: 'Case Studies', description: 'Engineering success through precision.' }, emptyState: { title: 'No projects found', description: 'Add your first project to get started!' } };
  ...
  return (
    <div className="min-h-screen pt-24 pb-24 px-6 bg-slate-50 dark:bg-[#050505] transition-colors duration-500">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        {isSectionVisible('portfolio', 'hero') && (
          <motion.div
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-cyan-500/10 via-purple-500/10 to-orange-500/10 border border-slate-200/60 dark:border-gray-700/50 backdrop-blur-xl mb-6">
              ...
              <span className="text-sm font-mono text-slate-600 dark:text-gray-400 uppercase tracking-wider">
                {PORTFOLIO_CONTENT?.header?.badge || 'OUR WORK'}
              </span>
            </div>
            ...
            <h1 className="text-4xl md:text-6xl font-mono font-bold mb-4 bg-gradient-to-r from-slate-900 via-slate-700 to-slate-900 dark:from-white dark:via-gray-300 dark:to-white bg-clip-text text-transparent">
              {PORTFOLIO_CONTENT?.header?.title || 'CASE STUDIES'}
            </h1>
            <p className="text-lg text-slate-600 dark:text-gray-400 max-w-2xl mx-auto">
              {PORTFOLIO_CONTENT?.header?.description || 'Engineering success through precision.'}
            </p>
          </motion.div>
        )}

        {/* Filters */}
        {isSectionVisible('portfolio', 'projects') && (
          <>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            {filteredProjects.length === 0 && (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">
                  {PORTFOLIO_CONTENT?.emptyState?.title || 'No projects found'}
                </h3>
                <p className="text-slate-600 dark:text-gray-400">
                  {PORTFOLIO_CONTENT?.emptyState?.description || 'Try selecting a different filter category'}
                </p>
              </motion.div>
            )}
          </>
